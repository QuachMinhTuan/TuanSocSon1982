...
<body>
            ...
            <div id="header-bootstrap" class="container-fluid fixed-top ">
                <div class="row no-gutters">
                    <div class="col-md-2 col-sm-2 col-2 py-2 text-center">
                        <a href="{{route('index')}}" id="Logo"><img src="{{ asset('public/image/logo.png') }}" alt=""
                                class="img-fluid"></a>
                    </div>
    ...
    product::where('parentlistproduct_id', '=', $parent->id)->where('disabler', '=', 'active')->get(),
                                                ...
                                                <li id="{{ $parent->id }}" class="menu-item">
                                                    <a href="{{ route('category',$parent->slug) }}" class="category-parent">
    ...
    product::where('productcat_id', '=', $productcat->id)->where('disabler', '=', 'active')->get(),
                                        ...
                                        <li>
                                            <a href="{{route('categoryproduct',$productcat->slug) }}"> {{ Str::of($productcat->catname)->title() }}</a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                        ...
                        <div class="banner">
                            <ul>
                                ...
                                <li class="mb-4 mt-4">
                                    <img src="{{asset($banner->img_banner)}}" alt="banner" class="img-fluid">
                                </li>
                                @endforeach
                    ...
                    <div class="col-md-9">
                                    ...
                                    <div id="home-slide" class="carousel slide">
                                        <ol class="carousel-indicators">
                                            ...
                                            @foreach ($sliders as $key => $slider)
                                                <li class="{{$key == 0 ? 'active hovercarasel' : 'hovercarasel'}}" data-target="#home-slide"
                        ...
                        <div class="container-fluid">
                            @foreach ($productcatscategory as $productcat)
                                @if (count(
    product::where('productcat_id', '=', $productcat->id)->where('disabler', '=', 'active')->get(),
) > 0)
                                    <section class="mt-3 mb-3 mx-2">
                                        <a href="{{route('categoryproduct',$productcat->slug)}}" class="category" title="{{Str::of($productcat->slug)->title() }}">{{ str::upper($productcat->catname) }}</a>
                                        <div class="row">
                                            @foreach ($products as $product)
                                                @if ($product->productcat_id == $productcat->id)
                                                    <div class="col-lg-3 col-md-4 col-sm-6 col-12 p-1 card-group">
                                                        <div class="card">
                                                            <a href="{{route('detailproduct',$product->slug_name)}}" class="detail-thumb" title="{{Str::of($product->slug_name)->title() }}"> <img
                                                                    src="{{asset($product->thumbnail )}}" alt="image"
                                                                    class="card-img img-fluid"></a>
                                                            <div class="card-body text-center p-3">
                                                                <a href="{{route('detailproduct',$product->slug_name)}}" class="card-title mb-1 name-product" title="{{Str::of($product->name)->title()  }}">{{Str::of($product->name)->title() }}</a>
                                                                <span class="ordered_buy">{{ number_format($product->price, 0, ',', '.') }}đ</span>
    ...
    product::where('parentlistproduct_id', '=', $parent->id)->where('disabler', '=', 'active')->get(),
                        ...
                        <li class="respon-menu-item">
                            <a href="{{route('category',$parent->slug) }}" title="{{Str::of($parent->slug)->title()}}" class="respon-category"> {{Str::of($parent->catparent)->title() }}</a>
    ...
    count(
        product::where('productcat_id', '=', $productcat->id)->where('disabler', '=', 'active')->get(),
                                        ...
                                        <li>
                                            <a href="{{route('categoryproduct',$productcat->slug)}}" title="{{Str::of($productcat->slug)->title()}}">{{Str::of($productcat->catname)->title() }}</a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
